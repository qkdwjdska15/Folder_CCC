...
public class TraineeDAO {
	...
	public void getTraineeRegiste(TraineeVO tvo) throws Exception{
		String sql = "insert into trainee" + "(no,sd_num,l_num,t_section,t_date" + "values" + "(trainee_seq.nexttval,?,?,?,sysdate)";
		Connection con = null;
		PreparedStatement pstmt = null;
	...
	public ArrayList<TraineeVO> getTraineeTotalList(String sd_num) throws Exception{
		...
		String sql ="select tr.no as no,sd_num,le.l_name as l_num, t_section, t_date" + "from trainee tr, lesson le" + "where tr.l_num = le.l_num and tr.sd_num = ?" + "order by t_date";
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		TraineeVO tVo = null; 
	...
	public boolean getTraineeDelete(int no) throws Exception {
		...
		try {
			...
			if(i == 1) {
				...
				alert.setHeaderText("수강신청 취소 완료");
				alert.setContentText("수강신청 취소 성공!!!!!!");
				alert.showAndWait();
				subjectDeleteSucess = true;
			}else {
				Alert alert = new Alert(AlertType.WARNING);
				alert.setTitle("수강신청 취소");
				alert.setHeaderText("수강신청취소 실패");
				alert.setContentText("수강신청취소 실패!!!!");
				alert.showAndWait();
			}
	...
	public ArrayList<TraineeVO> getTraineeTotalList() throws Exception{
		...
		String sql = "select tr.no, tr.sd_num, le.l_name as l_num,st,sd_name as sd_name, t_section,t_date" + "from traineetr,lesson le, student st" + "where tr.l_num = le.l_num and tr.sd_num order by t_date";
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		TraineeVO tVo = null;
		...
		try {
			...
			while(rs.next()) {
				...
				tVo.setL_num(rs.getString("l_num"));
				tVo.setT_section(rs.getString("t_section"));
				tVo.setT_date(rs.getString("t-date"));
		...
		public ArrayList<TraineeVO> getTraineeSubjectSearchList(String l_name) throws Exception {
			...
			String sql = "select tr.no as no,tr.sd_num,le.l_name as l_num,st.sd_nameas sd_name,t_section,t_date" + "from trainee tr, lesson le, student st" + "where tr.l_num = le.l_num and tr.l_num = ? and tr.sd_num = st.sd_num order by t_date";
